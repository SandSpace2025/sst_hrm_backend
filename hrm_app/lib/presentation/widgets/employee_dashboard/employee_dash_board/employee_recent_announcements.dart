...
class EmployeeRecentAnnouncements extends StatefulWidget {
  const EmployeeRecentAnnouncements({super.key});

  @override
  State<EmployeeRecentAnnouncements> createState() =>
      _EmployeeRecentAnnouncementsState();
}

class _EmployeeRecentAnnouncementsState
    extends State<EmployeeRecentAnnouncements> {
  @override
  Widget build(BuildContext context) {
    ...
    return Consumer<EmployeeProvider>(
      builder: (context, employeeProvider, child) {
        ...
        if (employeeProvider.isLoading && dashboardData == null) {
          return Container(
        ...
        List<dynamic> announcements = employeeProvider.announcements;

        if (announcements.isEmpty && dashboardData != null) {
          final dashboardAnnouncements = dashboardData['recentAnnouncements'];
          if (dashboardAnnouncements is List) {
            announcements = dashboardAnnouncements.map((item) {
              if (item is Map) {
  ...
  Widget _buildEnhancedAnnouncementItem(
    ...
    if (announcement is Map<String, dynamic>) {
      announcementMap = announcement;
    } else if (announcement is Map) {
      announcementMap = Map<String, dynamic>.from(announcement);
    } else {
    ...
    if (announcementMap['createdBy'] != null) {
      if (announcementMap['createdBy'] is Map<String, dynamic>) {
        final createdBy = announcementMap['createdBy'] as Map<String, dynamic>;
        author = createdBy['fullName'] ?? createdBy['name'] ?? 'Unknown';
      } else if (announcementMap['createdBy'] is Map) {
        final createdBy = Map<String, dynamic>.from(
          announcementMap['createdBy'] as Map,
        );
        author = createdBy['fullName'] ?? createdBy['name'] ?? 'Unknown';
      } else if (announcementMap['createdBy'] is String) {
        author = 'Unknown';
      }
    }
    ...
    return GestureDetector(
      onTap: () {
        _triggerHaptic();
        _showAnnouncementPreview(announcementMap);
      },
      child: Container(
