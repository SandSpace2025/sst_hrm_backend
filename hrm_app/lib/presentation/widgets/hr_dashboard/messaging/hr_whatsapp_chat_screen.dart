...
class HRWhatsAppChatScreen extends StatefulWidget {
  final String userId;
  final String userName;
  final String userType;
  final bool fromNotification;

  const HRWhatsAppChatScreen({
    super.key,
  ...
  @override
  State<HRWhatsAppChatScreen> createState() => _HRWhatsAppChatScreenState();
}

class _HRWhatsAppChatScreenState extends State<HRWhatsAppChatScreen>
    with TickerProviderStateMixin {
  HRProvider? _hrProvider;
  ...
  void initState() {
    ...
    _hrProvider = Provider.of<HRProvider>(context, listen: false);
    _hrProvider!.setCurrentChatPartner(widget.userId);
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      try {
        ...
        final online = wsProvider.isOnlineId(widget.userId);
  ...
  @override
  void dispose() {
    _hrProvider?.setCurrentChatPartner(null);
  ...
  Future<void> _ensureWebSocketConnected() async {
    try {
      ...
      final myRole = (authProvider.role ?? 'hr').toString();
      if (myId == null) return;
  ...
  void _extractMyUserId() async {
    try {
      ...
      if (authProvider.token != null) {
        ...
        await hrProvider.loadHRProfile(
          authProvider.token!,
          forceRefresh: false,
        );

        if (hrProvider.hrProfile != null) {
          _myUserId = hrProvider.hrProfile!.id;
        } else {
      ...
      (data) {
        ...
        try {
          ...
          final involvesMe =
              ...
              (normalizedJwtUserId.isNotEmpty &&
                  (normalizedSenderId == normalizedJwtUserId ||
                      normalizedReceiverId == normalizedJwtUserId));

          if (!isMyMessageCheck && (!involvesPartner || !involvesMe)) {
          ...
          final isMyMessage = isMyMessageCheck;
          ...
          if (mounted) {
            setState(() {
              if (tempMessageIndex != -1) {
                ...
                final exists = _localMessages.any(
                  (msg) =>
                      msg != null &&
  ...
  Future<void> _softReloadConversation() async {
    ...
    try {
      ...
      await hrProvider.getConversation(
        widget.userId,
        widget.userType,
        authProvider.token!,
        forceRefresh: false,
      ...
      setState(() {
        if (hrProvider.messages.isNotEmpty) {
          ...
          for (final message in hrProvider.messages) {
  ...
  Map<String, dynamic> _transformWebSocketData(Map<String, dynamic> data) {
    final senderModel =
        (data['sender']?['userType']?.toString() ?? '').isNotEmpty
        ? data['sender']['userType'].toString()
        : 'HR';
    final receiverModel =
        (data['receiver']?['userType']?.toString() ?? '').isNotEmpty
        ? data['receiver']['userType'].toString()
        : (widget.userType.toLowerCase() == 'admin' ? 'Admin' : 'Employee');
  ...
  Future<void> _markMessagesAsSeen() async {
    try {
      ...
      if (authProvider.token != null) {
        ...
        await hrProvider.markConversationAsSeen(
          widget.userId,
          widget.userType,
          authProvider.token!,
        );

        hrProvider.removeUnreadConversation(widget.userId);
        await _loadConversation();
      }
    } catch (e) {}
  }
  ...
  Future<void> _loadConversation() async {
    ...
    try {
      await hrProvider.getConversation(
        ...
        widget.userType,
        authProvider.token!,
        forceRefresh: widget.fromNotification,
      );
  ...
  Future<void> _sendMessage() async {
    ...
    final authProvider = Provider.of<AuthProvider>(context, listen: false);
    final payload = Jwt.parseJwt(authProvider.token!);
    final currentHRUserId = payload['userId'] ?? payload['id'] ?? '';

    final tempMessage = {
      '_id': 'temp_${DateTime.now().millisecondsSinceEpoch}',
      'content': message,
      'sender': {'userId': currentHRUserId, 'userType': 'HR'},
      'receiver': {'_id': widget.userId},
      'senderModel': 'HR',
      'receiverModel': widget.userType.toLowerCase() == 'admin'
          ? 'Admin'
          : 'Employee',
    ...
    try {
      if (widget.userType == 'employee') {
        await hrProvider.sendMessageToEmployee(
          widget.userId,
          'Chat Message',
          message,
          authProvider.token!,
        ...
        await hrProvider.sendMessageToAdmin(
          widget.userId,
          'Chat Message',
          message,
          authProvider.token!,
    ...
    if (!success) {
      setState(() {
        _localMessages.removeWhere((msg) => msg['_id'] == tempMessage['_id']);
      });
      _showErrorSnackbar('Failed to send message');
    } else {
      Future.delayed(const Duration(seconds: 2), () {
        if (mounted) {
          final hasTempMessage = _localMessages.any(
  ...
  bool _shouldShowDateHeader(int index, List<dynamic> messages) {
    ...
    try {
      final currentDate = DateTime.parse(messages[index]['createdAt'] ?? '');
  ...
  Widget _buildMessageBubble(dynamic message, bool isFromMe) {
    ...
    final isTempMessage = (message['_id'] ?? '').startsWith('temp_');
    ...
    try {
      ...
      time = DateFormat('hh:mm a').format(istTime);
    } catch (e) {
      time = '';
    }
