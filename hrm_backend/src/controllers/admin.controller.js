...
exports.getDashboardSummary = async (req, res) => {
  try {
    ...
    const [
      ...
      EOD.countDocuments({ reportStatus: { $regex: /^pending$/i } }),
      PayslipRequest.countDocuments({ status: "pending" }),
    ]);
    res.status(200).send({
      adminsOnline: adminCount,
      hrsOnline: hrCount,
      employeesOnline: employeeCount,
      pendingLeaveApprovals: pendingLeaveRequests,
  ...
  try {
    ...
    if (newUser && newUser._id) {
      await User.deleteOne({ _id: newUser._id }).catch((deleteErr) =>
        logger.error("Failed to cleanup user after error", {
...
exports.markConversationAsSeen = async (req, res) => {
  ...
  try {
    ...
    logger.error("Mark conversation as seen failed", {
      error: err.message,
      stack: err.stack,
      userId: userId,
      userType: userType,
