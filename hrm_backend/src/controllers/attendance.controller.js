...
exports.punchOut = async (req, res) => {
  try {
    ...
    const approvedLeave = await attendanceService.getApprovedLeave(userId, today);
...
exports.updateAttendance = async (req, res) => {
  try {
    ...
    // Recalculate Late Penalty (from first session)
    ...
    // Logic: Auto-update status if derived from duration
    ...
    if (approvedLeave && approvedLeave.leaveType === "work_from_home") {
      if (attendance.status !== "Present" && !status)
        attendance.status = "Present"; // Only force if not manually set?
      // Actually user says WFH treated as Present.
    }

    // Calculate Penalty (Centralized)
    attendanceService.calculateShortfallAndPenalty(attendance, approvedLeave);
