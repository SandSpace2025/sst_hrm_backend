...
const PayslipRequest = require("../models/payslipRequest.model"); // Added
...
exports.getEmployeeDashboardSummary = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error("Error checking pending EOD:", eodError);
    ...
    for (const announcement of recentAnnouncements) {
      try {
        ...
        console.error(
          "Error populating announcement creator:",
    ...
    console.error("Error getting employee dashboard summary:", err);
...
exports.getPayslips = async (req, res) => {
  try {
    ...
    console.error("Error getting payslips:", err);
    res.status(500).json({ message: err.message });
  }
};
...
exports.getSalaryBreakdownPreview = async (req, res) => {
  try {
    ...
    const components = payrollService.calculateSalaryComponents(basePay);

    // 5. Calculate Deductions (PT)
    const deductions = payrollService.calculateDeductions();
    ...
    const netSalary = basePay - penalty - deductions.pt - lopDeduction;
    ...
    console.error("Error getting salary preview:", err);
    res.status(500).json({ message: err.message });
  }
};
...
exports.getHRContacts = async (req, res) => {
  try {
    ...
    console.error("Error getting HR contacts:", err);
    res.status(500).json({ message: err.message });
  }
};

/**
 * @description Get admin contacts for messaging
 */
exports.getAdminContacts = async (req, res) => {
  try {
    ...
    console.error("Error getting admin contacts:", err);
    res.status(500).json({ message: err.message });
  }
};
...
exports.getEmployeeContactsForMessaging = async (req, res) => {
  try {
    ...
    console.error("Error getting employee contacts:", err);
    res.status(500).json({ message: err.message });
  }
};
...
exports.sendMessageToAdmin = async (req, res) => {
  try {
    ...
    // // console.log("🔍 SEND MESSAGE TO ADMIN - Starting request");
    // // console.log("  📊 Employee User ID:", employeeUserId);
    // // console.log("  📊 Recipient ID (Admin):", recipientId);
    ...
    // Permission gating removed: allow employees to message admin without prior approval
...
exports.getEmployeeMessages = async (req, res) => {
  try {
    ...
    console.error("Error getting employee messages:", err);
    res.status(500).json({ message: err.message });
  }
};
...
exports.getConversation = async (req, res) => {
  try {
    ...
    // Determine conversation id (Employee | HR/Admin)
    let convId;
    let a, b; // Declare for employee-to-employee use
    const lowerType = userType.toLowerCase();
    if (lowerType === "hr") {
      convId = `Employee:${employeeProfile._id}|HR:${userId}`;
    } else if (lowerType === "employee") {
      // Resolve peer as profile _id or base user id
      let peerProfileId = userId;
      try {
        ...
        if (!peerById) {
          ...
          if (peerByUser) {
            peerProfileId = peerByUser._id.toString();
          }
        }
      } catch (_) {}
      // Order-independent for employee-to-employee
      a = employeeProfile._id.toString();
      b = peerProfileId.toString();
      convId =
        a < b ? `Employee:${a}|Employee:${b}` : `Employee:${b}|Employee:${a}`;
    } else {
      convId = `Employee:${employeeProfile._id}|Admin:${userId}`;
    }

    // For employee-to-employee, also try reverse order for backward compatibility
    let conversation;
    if (lowerType === "employee") {
      const reversedConvId =
        a < b ? `Employee:${b}|Employee:${a}` : `Employee:${a}|Employee:${b}`;
      conversation = await Message.find({
        $or: [{ conversationId: convId }, { conversationId: reversedConvId }],
      })
        .sort({ createdAt: 1 })
        .lean();
    } else {
      conversation = await Message.find({ conversationId: convId })
        .sort({ createdAt: 1 })
        .lean();
    }
