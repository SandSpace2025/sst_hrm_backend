...
exports.createEOD = async (req, res) => {
  try {
    ...
    // TIME CONSTRAINT DISABLED FOR TESTING - EOD can be submitted anytime
    // Check current IST time
    // const currentIST = getISTTime();
    // const currentHour = currentIST.hour();
    // const currentMinute = currentIST.minute();

    // // Check if it's after 6:45 PM IST (18:45)
    // const isAfter645PM =
    //   currentHour > 18 || (currentHour === 18 && currentMinute >= 45);

    // if (!isAfter645PM) {
    //   return res.status(400).json({
    //     message: "EOD for today can be accessible only after 6:45 PM IST",
    //     currentTime: currentIST.format("hh:mm A"),
    //   });
    // }
...
exports.getTodayEOD = async (req, res) => {
  try {
    ...
    console.error("Error getting today's EOD:", err);
    res.status(500).json({ message: err.message });
  }
};
...
exports.getMyEODs = async (req, res) => {
  try {
    ...
    console.error("Error getting EODs:", err);
    res.status(500).json({ message: err.message });
  }
};
...
exports.getEmployeeEODs = async (req, res) => {
  try {
    ...
    console.error("Error getting employee EODs:", err);
    res.status(500).json({ message: err.message });
  }
};
...
exports.checkMissedEODs = async (req, res) => {
  try {
    ...
    console.error("Error checking missed EODs:", err);
    res.status(500).json({ message: err.message });
  }
};
...
exports.getEODStats = async (req, res) => {
  try {
    ...
    console.error("Error getting EOD stats:", err);
    res.status(500).json({ message: err.message });
  }
};
