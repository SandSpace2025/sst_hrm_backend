...
exports.getEmployeeLeaveData = async (req, res) => {
  try {
    ...
    logger.error("Error getting employee leave data", {
      error: err.message,
      stack: err.stack,
      userId: req.userId,
    });
...
exports.updateEmployeeLeaveData = async (req, res) => {
  try {
    ...
    logger.error("Error updating employee leave data", {
      error: err.message,
      stack: err.stack,
...
exports.getHRDashboardSummary = async (req, res) => {
  try {
    ...
    logger.error("HR Dashboard Summary Error", {
      error: error.message,
      stack: error.stack,
      userId: req.userId,
    });
...
exports.getHRProfile = async (req, res) => {
  try {
    ...
    logger.error("Error getting HR profile", {
      error: err.message,
      stack: err.stack,
      userId: req.userId,
    });
...
exports.updateHRProfile = async (req, res) => {
  try {
    ...
    logger.error("Error updating HR profile", {
      error: err.message,
      stack: err.stack,
      userId: req.userId,
    });
...
exports.createEmployee = async (req, res) => {
  try {
    ...
    const newEmployee = new Employee({
      ...
      joinDate: req.body.joinDate || null, // Added joinDate
    });
...
exports.updateEmployee = async (req, res) => {
  try {
    ...
    if (email) updateData.email = email;
    if (bloodGroup) updateData.bloodGroup = bloodGroup;
    if (payrollInfo) updateData.payrollInfo = payrollInfo; // Allow updating payroll info
    if (joinDate) updateData.joinDate = joinDate; // Allow updating join date
...
exports.getEmployeesForMessaging = async (req, res) => {
  try {
    ...
    const employees = await Employee.find(query)
      .select(
        "name email employeeId phone jobTitle designation subOrganisation",
      )
      .sort({ name: 1 })
      .skip(skip)
      .limit(parseInt(limit));
    ...
    logger.error("Error getting employees for messaging", {
      error: err.message,
      stack: err.stack,
    });
    res.status(500).json({ message: err.message });
...
exports.getAdminsForMessaging = async (req, res) => {
  try {
    ...
    logger.error("Error getting admins for messaging", {
      error: err.message,
      stack: err.stack,
    });
    res.status(500).json({ message: err.message });
...
exports.sendMessageToEmployee = async (req, res) => {
  try {
    ...
    // --- Start Notification Logic (Async - Non-blocking) ---
    if (employee.user) {
      (async () => {
        try {
          ...
          if (user && user.fcmToken) {
            await notificationService.sendNotification(
              user.fcmToken,
              `New Message from ${hrProfile.name}`,
              content.substring(0, 100) + (content.length > 100 ? "..." : ""),
              {
                type: "message",
                conversationId: conversationId,
                senderId: hrProfile._id.toString(),
                senderName: hrProfile.name,
              },
            );
          }
        } catch (notifErr) {
    ...
    if (admin.user) {
      (async () => {
        try {
          const notificationService = require("../services/notification.service");
          const User = require("../models/user.model");
          const user = await User.findById(admin.user).select("fcmToken");
...
exports.getHRMessages = async (req, res) => {
  try {
    ...
    logger.error("Error getting HR messages", {
      error: err.message,
      stack: err.stack,
    });
    res.status(500).json({ message: err.message });
...
exports.getConversation = async (req, res) => {
  try {
    ...
    // Use robust query instead of brittle conversationId string
    const lowerType = userType.toLowerCase();
    const targetModel = lowerType === "employee" ? "Employee" : "Admin";

    // Robustly resolve user ID if needed (similar to employee controller)
    // For now assuming IDs are correct as they come from params

    // console.log(`[HR] Fetching conversation with ${targetModel} ${userId}`);

    const conversation = await Message.find({
      $or: [
        {
          "sender.userId": hrProfileId,
          "sender.userType": "HR",
          receiver: userId,
  ...
  try {
    ...
    const announcementData = {
      ...
      createdBy: {
        ...
        userId: hrProfile.user.toString(), // Include User ID for frontend filtering
        fullName:
    ...
    try {
      ...
      // EXCLUDE the sender (the HR who created it)
      targetQuery._id = { $ne: hrUserId };

      // Fetch users matching the audience
      const targetUsers = await User.find(targetQuery).select("_id fcmToken");

      if (targetUsers.length > 0) {
        // 1. Bulk Insert DB Notifications
        ...
        if (tokens.length > 0) {
          // Since we already inserted DB notifications, we just need to send push
          // We can use the multicast send directly via FCM admin or a specific service method
          // For consistency with announcement.controller.js, we assume the same logic
      ...
      logger.error("Failed to send HR announcement notifications", {
        error: notifErr.message,
      });
    }
    // --- End Notification Logic ---
      ...
      if (hrProfile.profilePic) {
        ...
        if (fs.existsSync(oldImagePath)) {
          try {
            ...
            logger.error("Error deleting old HR profile image", {
