  ...
  async (req, res) => {
    try {
      ...
      try {
        ...
        // Use a Set to ensure unique users (though _id is unique, just in case of multiple queries if expanded)
        // Actually, just iterating the list is fine as User.find returns unique docs.

        // console.log(
        //   `Creating notifications for ${approvers.length} approvers...`,
        // );

        for (const approver of approvers) {
          if (approver.fcmToken) {
            // Fire and forget individual notifications to avoid blocking
            notificationService
              .sendNotification(
                approver.fcmToken,
                "New Leave Request",
                ...
                approver._id, // Pass user ID for persistence
                true, // useDataOnly = true
              )
              .catch((e) =>
                logger.error(`Failed to notify user ${approver._id}`, {
                  error: e.message,
                }),
              );
          }
        ...
        logger.error("Failed to send new leave request notification", {
...
exports.getBlackoutDates = async (req, res) => {
  try {
    ...
    logger.error("Error getting blackout dates", {
      error: err.message,
      stack: err.stack,
    });
    res.status(500).json({ message: err.message });
...
exports.getLeaveStatistics = async (req, res) => {
  try {
    ...
    const pendingPermissionDeduction =
      (
        await LeaveRequest.aggregate([
          {
            $match: {
    ...
    const remainingWFHLeaves = Math.max(
      0,
      totalWFHLeaves - pendingWFHDeduction,
    );
    const remainingPermissionHours = Math.max(
      0,
