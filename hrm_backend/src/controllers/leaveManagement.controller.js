...
// Helper: Send notification (reuse across functions)
const sendLeaveNotification = async (
  ...
  try {
    // We need the Employee document to access the 'user' reference
    // Check if employee is already populated; if so, and it has an _id, use that.
    ...
    await notificationService.sendNotification(
      ...
      body,
      {
        ...
        status: statusOverride || leaveRequest.status,
        leaveType: leaveRequest.leaveType,
      },
      user._id,
      true, // useDataOnly = true
    );
  } catch (error) {
    logger.error("Error sending leave notification", {
      error: error.message,
    });
  }
};
...
const resetMonthlyLeaveBalances = async (employeeId) => {
  ...
  if (now.diff(lastReset, "months") >= 1) {
    ...
    // Add 1 casual leaves for the new month (carries over)
...
exports.approveLeaveRequestHR = async (req, res) => {
  try {
    ...
    if (leaveRequest.requiresBothApprovals) {
      ...
      if (
        ...
        await sendLeaveNotification(
          ...
          `Your ${leaveRequest.leaveType} leave request has been approved by HR and is awaiting Admin approval.`,
          "pending_admin",
        );
    ...
    logger.error("Error approving leave request (HR)", {
      error: err.message,
      stack: err.stack,
      userId: req.userId,
    });
...
exports.rejectLeaveRequestHR = async (req, res) => {
  try {
    ...
    await sendLeaveNotification(
      leaveRequest,
      "Leave Rejected",
      `Your ${leaveRequest.leaveType} leave request was rejected by HR. Reason: ${rejectionReason}`,
    );
    ...
    logger.error("Error rejecting leave request (HR)", {
      error: err.message,
      stack: err.stack,
      userId: req.userId,
    });
...
exports.approveLeaveRequestAdmin = async (req, res) => {
  try {
    ...
    if (leaveRequest.requiresBothApprovals) {
      ...
      if (
        ...
        await sendLeaveNotification(
          leaveRequest,
          "Leave Request Update",
          `Your ${leaveRequest.leaveType} leave request has been approved by Admin and is awaiting HR approval.`,
          "pending_hr",
        );
    ...
    logger.error("Error approving leave request (Admin)", {
      error: err.message,
      stack: err.stack,
      userId: req.userId,
    });
...
exports.rejectLeaveRequestAdmin = async (req, res) => {
  try {
    ...
    await sendLeaveNotification(
      leaveRequest,
      "Leave Rejected",
      `Your ${leaveRequest.leaveType} leave request was rejected by Admin. Reason: ${rejectionReason}`,
    );
    ...
    logger.error("Error rejecting leave request (Admin)", {
      error: err.message,
      stack: err.stack,
      userId: req.userId,
    });
...
exports.cancelOngoingLeave = async (req, res) => {
  try {
    ...
    await sendLeaveNotification(
      leaveRequest,
      "Leave Cancelled",
      `Your ongoing ${leaveRequest.leaveType} leave was cancelled by ${userRole === "hr" ? "HR" : "Admin"}. Reason: ${cancellationReason}`,
    );
    ...
    logger.error("Error cancelling ongoing leave", {
      error: err.message,
      stack: err.stack,
      userId: req.userId,
    });
...
exports.createBlackoutDate = async (req, res) => {
  try {
    ...
    logger.error("Error creating blackout date", {
      error: err.message,
      stack: err.stack,
      userId: req.userId,
    });
...
exports.updateBlackoutDate = async (req, res) => {
  try {
    ...
    logger.error("Error updating blackout date", {
      error: err.message,
      stack: err.stack,
      userId: req.userId,
    });
...
exports.deleteBlackoutDate = async (req, res) => {
  try {
    ...
    logger.error("Error deleting blackout date", {
      error: err.message,
      stack: err.stack,
      userId: req.userId,
    });
...
exports.getEmployeeLeaveSummary = async (req, res) => {
  try {
    ...
    logger.error("Error getting employee leave summary", {
      error: err.message,
      stack: err.stack,
