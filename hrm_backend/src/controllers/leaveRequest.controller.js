...
const recalculateAttendanceForLeave = async (leaveRequest) => {
  try {
    ...
    logger.error("Error recalculating attendance for leave", {
      error: err.message,
...
const updateLeaveRequestStatus = async (req, res) => {
  try {
    ...
    // --- Start Notification Logic (Moved After WebSocket) ---
    try {
      ...
      if (updatedLeaveRequest.employee && updatedLeaveRequest.employee.user) {
        ...
        if (user && user.fcmToken) {
          ...
          // Fire and forget - do not await strictly if we want faster strict response,
          // but awaiting here is okay since socket is already sent.
          // Better to keep it non-blocking or just log error.
          ...
          notificationService
            .sendNotification(
              ...
              body,
              {
                ...
                leaveRequestId: updatedLeaveRequest._id.toString(),
                status: status,
                leaveType: updatedLeaveRequest.leaveType,
              },
              user._id, // Explicit user ID if available
              true, // useDataOnly = true
            )
            .catch((notifErr) => {
              logger.error("Failed to send leave update notification", {
...
const bulkUpdateLeaveRequests = async (req, res) => {
  try {
    ...
    for (const leaveReq of updatedRequests) {
      await recalculateAttendanceForLeave(leaveReq);
      ...
      try {
        ...
        // Fetch employee user to get token
        if (leaveReq.employee && leaveReq.employee.user) {
          const user = await User.findById(leaveReq.employee.user).select(
            "fcmToken",
          );
          if (user && user.fcmToken) {
            let title = "Leave Request Update";
            ...
            notificationService
              .sendNotification(
                ...
                {
                  type: "leave_request",
                  id: leaveReq._id.toString(),
                  status: status,
                },
                user._id, // Explicit user ID if available
                true, // useDataOnly = true (consistent with payslip pattern)
              )
              .catch((notifErr) => {
                logger.error("Failed to send bulk leave update notification", {
                  error: notifErr.message,
                });
              });
          }
    ...
    logger.error("Error bulk updating leave requests", {
      error: error.message,
      stack: error.stack,
    });
    res.status(500).json({ message: "Internal server error" });
...
const getLeaveRequestStats = async (req, res) => {
  try {
    ...
    logger.error("Error fetching leave request stats", {
      error: error.message,
      stack: error.stack,
    });
    res.status(500).json({ message: "Internal server error" });
...
const getLeaveRequestsByEmployee = async (req, res) => {
  try {
    ...
    logger.error("Error fetching leave requests by employee", {
      error: error.message,
      stack: error.stack,
...
const updateEmployeeLeaveBalance = async (leaveRequest, action) => {
  try {
    const employee = await Employee.findById(leaveRequest.employee);
    if (!employee) {
      logger.error("Employee not found for leave request", {
        leaveRequestId: leaveRequest._id,
      });
      return;
    }
    ...
    logger.error("Error updating employee leave balance", {
      error: error.message,
      leaveRequestId: leaveRequest._id,
    });
  }
