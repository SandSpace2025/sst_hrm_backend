...
const recalculateAttendance = async (userId, date) => {
  try {
    ...
    if (!attendance) {
      // If no attendance record, implies Absent?
      // But if Leave is approved (e.g. Full Day CL), maybe we should CREATE a record?
      // User says "Payroll consumes final attendance".
      // If CL approved, but no attendance record exists, Payroll sees nothing.
      // It skips the day. Result: Full Pay (No Deduction). CORRECT for CL.
      // If SL (No Balance) approved? Payroll sees nothing. Result: Full Pay. INCORRECT (Should be LOP).
      // So for UNPAID leave (LOP), we might need an 'Absent' record?
      // But Payroll `calculateLOPDays` logic relies on `status: 'Absent'`.
      // If I approve an LOP Leave, I should create an Attendance Record with status 'Absent'??
      // For now, let's stick to updating EXISTING attendance.
      return;
    }
