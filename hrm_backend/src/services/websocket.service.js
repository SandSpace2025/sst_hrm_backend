...
class WebSocketService {
  ...
  async handleAuthenticatedConnection(socket, user) {
    ...
    // Send existing online users to the new user so they know who is online
      ...
      this.io.sockets.sockets.forEach((s) => {
        if (s.userId && s.id !== socket.id) {
          socket.emit("user_connected", {
            ...
            name: s.userRole ? "User" : undefined,
            role: s.userRole,
          });
        }
      });
  ...
  async assignUserToRooms(socket, user) {
    ...
    // logger.info(`User joined rooms`, { userId: user._id, rooms });
  }
  ...
  joinRoom(socket, roomName) {
    ...
    // logger.debug(`User joined room`, { userId: socket.userId, room: roomName });
  ...
  leaveRoom(socket, roomName) {
    ...
    // logger.debug(`User left room`, { userId: socket.userId, room: roomName });
  ...
  handleCustomEvent(socket, data) {
    // logger.debug(`Custom event from ${socket.userId}`, { data });
  ...
  broadcastToUser(userId, event, data) {
    ...
    if (socketId && this.io) {
      try {
        ...
        // logger.debug(`Successfully sent ${event} to user ${userId}`);
      } catch (error) {}
    } else {
  ...
  broadcastToAll(event, data) {
    if (this.io) {
      try {
        ...
        // logger.debug(`Broadcasted '${event}' to all connected users`);
      } catch (error) {}
    }
  }
